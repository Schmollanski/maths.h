#pragma once
#include <iostream>

#define Nr1Teamate(...) main() {__VA_ARGS__}
#define Nr1TeamateButMomosuzuNeneUnfortunatelyCommitedSuicide(a, b) a Nr1Teamate(b)
#define ohImDieThankYouForeva(...) for(__VA_ARGS__)
#define ohImDieThankYouForevaSuperCitizenEdition(a, b) for(a) { b }
#define bismillah(a,b,c) a b c
#define fubukus ,
#define KFPEmployee ;
#define mathJumpscare int
#define X_Potato ?
#define TheForbiddenWah std::cout
#define TheySaidICouldBecomeAnything_TakoMeshLaundryPouch <<
#define BibooTheRizzlerSays TheForbiddenWah TheySaidICouldBecomeAnything_TakoMeshLaundryPouch
#define TotallyShef_Apron >>
#define Bonk_Bonk_Bonk std::string
#define YATAAAAA_AAAAAAA std::endl
#define NerizzlerAnswers std::cin
#define Nyahello :
#define KAELAAAAAAAAAAAAAAA +=
#define BIBBOOOOOOOOOOOOOOO -=
#define WindowWhiper !=
#define SSRBShoppingBasket +
#define _35P ++
#define MikoInMaguma ==
#define AmericaYaaa =
#define Hello_Hello_Hello_Hello <
#define HairballKorone *
#define momosuzunene {}
#define nenechi void
#define NENESealInsulatedBottle 0
#define BEEGsmol 1
#define FuwamocoRadio 2
#define smileFumo ""
#define cirnoStare " "
#define BoyoyoyooBoyoyoyoPSfpsfpsfpspfpsf return
#define dasdingiiiist(a,b)  if(a) { b }
#define TheMioNFamily(a) else{a}
#define TheOtherMioNFamily else
#define GuraGoomba(a, b) TheOtherMioNFamily dasdingiiiist(a, b)
#define UmAhemWellWhenIWatch_UhWhenIC_WhenIConsu_WhenIUhPass_WhenISeeUmH_H_Hen_WhenISeeArtUmIPer_PersonallyUhYouKnowTh_TheBigOlBadonk_TheBigOlTiddbgelgjlrkjBigBigUhImNotAHugeFanOfTheYouKnow_YouKnowUhTheBigUhYwe_UhAhemSi_SizeUhAhemUhUhSizeUhLUhAAAAHILikeEmSm_SmallerYeahNotToo_NotToo_OkayNextQuestion(a, b, c, d) dasdingiiiist(a, b) GuraGoomba(c, d)
#define SchwarzeneggerFamilie(a, b, c, d, e) dasdingiiiist(a, b) GuraGoomba(c, d) TheMioNFamily(e)
#define ShishiroBotanBirthday2021SSRBAmusementParkEntranceContainerBOX false
#define ShishiroBotan3rdAnniversaryCelebrationSSRBShoppingBasket true
#define IHaveGottenAWordFromASettlement(a,b) while(a){b}
#define DankeschoenWirSindDieCantinaBandWennIhrSongwuenscheHabtRuftSieEinfachSpieltDenSelbenSongNochmalAllesKlarDenSelbenSongUndLos(b) IHaveGottenAWordFromASettlement(ShishiroBotan3rdAnniversaryCelebrationSSRBShoppingBasket, b)
